        ...
        return (
            <div className="min-h-screen bg-background text-foreground flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-foreground/60">Loading team statistics...</p>
                </div>
            </div>
        );
    }
            ...
            <div className="max-w-7xl mx-auto space-y-12">
                ...
                <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div>
                        ...
                        <p className="text-foreground/60 mt-2 text-sm">Comprehensive team performance analytics</p>
                    </div>

                    {/* Sport Selector */}
                    <div className="flex bg-muted p-1 rounded-2xl border border-border gap-1">
                        {['Football', 'Basketball'].map((sport) => (
                            <button
                                key={sport}
                                onClick={() => setSelectedSport(sport as any)}
                                className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${selectedSport === sport ? 'bg-primary text-primary-foreground' : 'text-foreground/40 hover:text-foreground'
                                    }`}
                            >
                                {sport}
                            </button>
                ...
                {teamStats.length > 0 ? (
                    <div className="space-y-6">
                        {teamStats.map((stats, index) => (
                            <motion.div
                                ...
                                className="bg-muted border border-border rounded-3xl p-8 hover:bg-muted/80 transition-all"
                            >
                                {/* Team Header */}
                                <div className="flex items-center justify-between mb-8">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-foreground/60 font-bold uppercase tracking-wider">{stats.team.sport}</p>
                                        </div>
                                    </div>
                                    <Link
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                                    ...
                                    <div className="bg-muted border border-border rounded-2xl p-6">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <p className="text-xs text-foreground/60">{stats.splits.home.played} matches played</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-muted border border-border rounded-2xl p-6">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <p className="text-xs text-foreground/60">{stats.splits.away.played} matches played</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4 mt-6">
                                    <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-foreground/40 mb-1">Longest Win Streak</p>
                                        <p className="text-3xl font-display italic text-green-500">{stats.advanced.longestWinStreak}</p>
                                    </div>
                                    <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-foreground/40 mb-1">Longest Loss Streak</p>
                    ...
                    <div className="text-center py-20">
                        <TrendingUp size={64} className="mx-auto text-foreground/10 mb-4" />
                        <h2 className="text-2xl font-bold mb-2">No Statistics Available</h2>
                        <p className="text-foreground/40">No team statistics found for {selectedSport}</p>
                    </div>
                )}
            </div>
        </div>
...
function StatRow({ label, value, highlight, large }: { label: string; value: string | number; highlight?: boolean; large?: boolean }) {
    return (
        <div className="flex items-center justify-between">
            ...
            <span className={`${large ? 'text-2xl' : 'text-sm'} font-display italic ${highlight ? 'text-primary' : 'text-foreground'}`}>
                {value}
            </span>
        </div>
    );
