...
export function SearchOverlay({ onClose, onSelectTeam, onSelectPlayer }: SearchOverlayProps) {
  ...
  const filteredPlayers = useMemo(() => {
    let result = PLAYERS.filter(p => {
      ...
      // Sport filtering would happen here if players had sport metadata
      return matchesQuery;
    });

    return result.sort((a, b) => {
      if (sortBy === 'rating') return b.rating - a.rating;
      if (sortBy === 'points') return b.eyePoints - a.eyePoints;
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] bg-secondary/10 text-secondary px-1.5 py-0.5 rounded font-black italic">
                          {sortBy === 'points' ? player.eyePoints : player.rating.toFixed(1)}
                        </span>
                      </div>
                    ...
                    <button 
                      ...
                      className={`p-3 rounded-xl transition-colors ${isFavoritePlayer(player.id) ? 'text-secondary bg-secondary/10' : 'text-white/20 hover:text-white hover:bg-white/10'}`}
                    >
                      <Heart size={20} fill={isFavoritePlayer(player.id) ? "currentColor" : "none"} />
                    </button>
                </div>
              ))}
            </div>
