...
export default function MatchPoll({ matchId, userId, liveUpdates = true }: MatchPollProps) {
                        ...
                        <div className="p-6 space-y-3">
                            <AnimatePresence mode="wait">
                                {poll.options.map((option) => {
                                    ...
                                    const isLeading = poll.totalVotes > 0 && option.votes === maxVotes;
                                    const showResults = poll.totalVotes > 0; // Show results if there are any votes
                                    ...
                                    return (
                                        <motion.button
                                            ...
                                            className={`
                                                ...
                                                ${isLeading && showResults ? 'ring-2 ring-blue-500/50' : ''}
                                            `}
                                            whileHover={!userVoted ? { scale: 1.02 } : {}}
                                            whileTap={!userVoted ? { scale: 0.98 } : {}}
                                        >
                                            {/* Progress Bar - Show if there are votes */}
                                            {showResults && (
                                                <motion.div
                                                    initial={{ width: 0 }}
                                            ...
                                            <div className="relative flex items-center justify-between">
                                                ...
                                                {showResults && (
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <span className={`text-xl font-bold min-w-[4rem] text-right ${isLeading ? 'text-blue-400' : 'text-white'
                                                            }`}>
                                                            {percentage}%
                                                        </span>
                                                    </div>
