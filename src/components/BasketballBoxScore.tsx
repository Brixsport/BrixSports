    ...
    if (home.length === 0 && away.length === 0) {
        return (
            <div className="text-center py-20">
                ...
                <h3 className="text-xl font-semibold text-foreground/60 mb-2">No box score available</h3>
                <p className="text-foreground/40">Player stats will appear here once the game starts</p>
            </div>
        );
    }
        ...
        <div key={team.id} className="mb-6">
            ...
            <div className="overflow-x-auto rounded-xl border border-border">
                <table className="w-full text-sm">
                    <thead>
                        <tr className="bg-muted text-foreground/60 text-[10px] uppercase tracking-wider">
                    ...
                    <tbody>
                        {rows.length === 0 ? (
                            <tr>
                                <td colSpan={4} className="px-3 py-4 text-center text-foreground/40">No player events yet</td>
                            </tr>
                        ) : (
                            rows.map(r => (
        ...
        <div className="max-w-3xl mx-auto">
            <div className="flex items-center justify-center gap-3 mb-6">
                <button
                    ...
                    className={`p-2 rounded-full transition-all ${filter === 'home' ? 'bg-primary/20 ring-2 ring-primary' : 'bg-muted hover:bg-muted/70'}`}
                ...
                <button
                    ...
                    className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide transition-all ${filter === 'all' ? 'bg-primary text-primary-foreground' : 'bg-muted text-foreground/60 hover:text-foreground'}`}
                >
                    All
                </button>
                <button
                    onClick={() => setFilter('away')}
                    className={`p-2 rounded-full transition-all ${filter === 'away' ? 'bg-primary/20 ring-2 ring-primary' : 'bg-muted hover:bg-muted/70'}`}
