...
**Net assessment:** 2 of 3 pieces solid (real-time presence, and correct fast event/score persistence — the latter was arguably already true pre-fix via the REST route, so isn't new evidence for *this* fix specifically). The one piece that would actually distinguish "the WS push works" from "still falling back to the old poll" — an event appearing on another logger's screen well under 10s — was not confirmed either way. **Next session: re-run with a longer observation window (20-30s) and explicit differentiation between "arrived via push" (fast) vs "arrived via poll" (~10-15s, the old mechanism) rather than a single pass/fail — that distinction is the actual open question now, not whether the feature works at all.**
