...
- **BUG-074** _(HIGH — Staging / Security)_: Staging and prod share the same Railway WebSocket server instance. Originally filed as "wrong WS URL in staging env vars" — root cause is deeper: there is only one Railway service, so staging and prod are on the same Socket.IO process. Confirmed consequences (2026-07-01):
  ...
  **Still explicitly NOT fixed by this**: the shared `JWT_SECRET` across environments (a staging-issued token remains valid on prod API routes right now) — untouched, unrelated to WS rooms. The originally recommended proper fix (a second, independent Railway service for staging) also still hasn't been done — this is the workaround, now complete, not the fix.
  ...
  **Status:** BUG-074 stays OPEN — this closes the specific live-broadcast leakage risk demonstrated by BUG-108's testing this session, not the bug's full original scope (JWT_SECRET sharing untouched, no second Railway service). Cross-environment isolation itself is unverified live — flag for whenever a real dual-environment test is safe to run (e.g. a scheduled prod match with a simultaneous staging smoke test).
...
- **BUG-121** _(HIGH — Data Integrity / Scoring, session 44, found while scoping single-writer enforcement, filed separately per Richard's call)_: **`POST /api/matches/[id]/events`'s score update is a read-modify-write race, and the whole handler (event insert + score update) has no enclosing transaction.**
  ...
  **Fix (not yet built)**: score update should use an atomic SQL increment (e.g. Drizzle's `sql\`${matches.homeScore} + ${points}\`` in the `.set()` call, not a pre-computed JS value) to close the race regardless of concurrent writers; the insert + score update (+ player stats update, same pattern) should be wrapped in a single `db.transaction(async (tx) => {...})` so a failure partway through doesn't leave a partial write. `DELETE /api/matches/[id]/events/[eventId]`'s score-revert path (same file family) likely has the identical read-modify-write shape and should be checked/fixed in the same pass.
  **Status:** OPEN — filed, not fixed, not live-reproduced (confirmed via code read only).
...
- **BUG-122** _(HIGH — Real-Time / Live Clock, session 44, single-writer enforcement — `SYSTEM_CRITICALITY_MAP.md` §5 / `LIVE_CLOCK_V2_ARCHITECTURE.md` §5, unblocked by BUG-120)_: **Two loggers both connected to the same match each ran their own independent clock and both broadcast `match:time:update`, producing visible flicker for viewers as the two competing sources arrived out of order.** Not a wrong-data bug — both loggers are each roughly correct — a "two voices" UX problem, not a clock-algorithm problem.
  ...
  **Status:** SHIPPED locally, not yet deployed or live-tested with two real simultaneous loggers — needs a push + live dual-logger verification before RESOLVED.
