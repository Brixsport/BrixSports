...
- **BACKLOG-105** _(HIGH — Data Integrity)_: Penalty shootout score isolation. Full implementation. **Status:** SHIPPED — 2026-08-04 (session 48), core mechanics live-verified; the 3 gaps below (`BUG-197`, `BACKLOG-190`, `BACKLOG-191`) fixed session 51 continuation, 2026-08-20 — see each for evidence.
  ...
  **`BUG-197`** _(Real-Time)_: **Status:** RESOLVED — 2026-08-20, live-verified against staging. **Root cause was never the broadcast pipeline** — traced the full path (DB write → `broadcastScoreUpdate` → `after()` → HTTP POST to the Railway WS server → room-scoped `io.to().emit()` → client `on('match:score:updated')` listener) and found it structurally identical to, and sharing every function with, the already-proven-working main-score path. The actual bug was in the **render condition**: `hasShootoutResult = shootoutHomeScore != null && shootoutAwayScore != null && shootoutHomeScore !== shootoutAwayScore`, in both `matches/[id]/page.tsx` and `MatchCard.tsx`. `shootout_home_score`/`shootout_away_score` default to `0` (not `null`) on every match, shootout or not (`schema.ts`) — so the `!==` inequality check was the *only* thing gating display, and it hid the live PEN line every time an in-progress shootout was tied, which is most of a real shootout (1-1, 2-2, etc.). State was updating correctly the whole time; the UI just went invisible at every tied moment, which looked exactly like "stuck, needs a reload" whenever the last-observed state happened to be a tie. **Fix:** added a `isLiveShootout` check (`currentPeriod === 'PENALTY_SHOOTOUT'` / `status === 'PENALTY_SHOOTOUT'`) that shows the running score regardless of tie state while the shootout is actually live; the inequality check still applies once `FINISHED`, since a real shootout can never legitimately end tied — that part of the original reasoning was correct, it just shouldn't have applied to the in-progress case too.
  **Evidence:** `tsc --noEmit` clean, zero new errors. **Real staging test**, session 51 continuation, commit `76cd569` deployed to `brixsports-staging.vercel.app`: created a throwaway match, drove it live through `SECOND_HALF → EXTRA_TIME_1 → EXTRA_TIME_2 → PENALTY_SHOOTOUT` (see `BACKLOG-191` below), opened the match detail page in a browser tab **before firing any kicks** — the exact scenario originally reported — then fired real `PEN_SCORED` events via the API one at a time: 1-0 (showed live, no reload), **1-1 — tied, showed live** (this is the exact state the pre-fix code would have rendered nothing for), 2-1 (showed live). Confirmed via `get_page_text` reads of the same open tab after each kick, no navigation/refresh between them. Throwaway match + its 3 events deleted in cleanup, confirmed via post-delete count.

  **`BACKLOG-190`** _(Data Integrity — real rule violation, not cosmetic)_: **Status:** FIX APPLIED — 2026-08-20, tsc-clean, algorithm-verified, **not click-tested through the real logger UI** (see below for why). `ShootoutModal`'s taker picker has no memory of who has already kicked. Confirmed via research this session against the actual IFAB Law 10.3 text: *"Each kick is taken by a different player, and all eligible players must take a kick before any player can take a second kick."* The modal currently shows the full on-pitch roster for every single kick with zero tracking — nothing stops a logger from selecting the same taker twice in a row, which is a real rules violation, not just a UX nicety.
  **Fix:** eligibility state (`takenThisRound: {home: Set, away: Set}`) lives in `FootballLogger.tsx`, not inside `ShootoutModal` itself — the modal unmounts between kicks (`showShootoutModal && <ShootoutModal .../>`) and would otherwise forget who'd already gone every time it closes. On each *confirmed-saved* kick (only inside the `res.ok` branch — a failed POST leaves the taker eligible, since the kick was never actually recorded), the taker's id is added to their team's set; once a team's set covers its whole on-pitch roster, it's cleared immediately (round complete → sudden-death repeats become allowed again, per the same rule). The picker passed into `ShootoutModal` is the on-pitch roster filtered against this set, with a defensive fallback to the full roster if filtering would ever leave zero eligible players (shouldn't happen with a real 11-a-side squad, but an empty picker would soft-lock the logger mid-shootout, which is worse than the rule being unenforced for one kick).
  **Why not click-tested:** this is pure client-side React state with no server-side enforcement (matches how the fix was scoped — a UI guard, not a new API validation) — the only way to observe it is rendering the actual component and reading the picker's contents, which requires driving the real logger UI through a full period-transition sequence plus several shootout kicks via browser automation. Judged not worth the time this session given the local dev server was unstable and a staging click-through carries real flakiness risk for marginal proof beyond the algorithm itself. **Algorithm-verified instead:** a standalone script replicating the exact Set-based round-tracking logic against a scripted 11-a-side kick sequence confirmed correct exclusion after each kick and correct reset exactly on the 12th kick (start of sudden death). Flagged as a real gap, not silently marked RESOLVED.

  **`BACKLOG-191`** _(Test coverage gap)_: **Status:** RESOLVED (partial) — 2026-08-20, live-verified against staging. Session 48's live verification jumped directly into a match seeded at `PENALTY_SHOOTOUT` (via direct DB insert) rather than testing the real full pipeline — normal time (0-0 or tied) → extra time → still tied → shootout. **This session:** drove a real throwaway tied match (1-1) through the actual `PATCH /api/matches/[id]` transitions `FootballLogger.tsx` fires — `SECOND_HALF` → `EXTRA_TIME_1` → `EXTRA_TIME_2` → `PENALTY_SHOOTOUT` — against staging (as admin; a logger token 403'd on the first attempt since the throwaway match had no real `match_logger_assignments` row, only a `logger_id` column — not a bug, just a test-setup gap, admin bypasses the assignment check by design). All 3 PATCHes `200`'d, final DB row confirmed `current_period: 'PENALTY_SHOOTOUT'`, `status: 'LIVE'`. This exercises the server-side period-transition path end-to-end, which is what this entry was actually worried about (the older, previously-tested `TD-010` logic hadn't been re-confirmed reachable since this session's shootout changes landed) — then continued straight into the same throwaway match for `BUG-197`'s live open-tab test above, so the full normal-time-adjacent → shootout → live-display chain was exercised together, not just the transition in isolation. **Not covered:** the literal UI button-click sequence through `FootballLogger.tsx`'s period-transition confirms (tested via direct API PATCH, matching what those buttons call, not by clicking them) — same reasoning as `BACKLOG-190` above for not doing a full UI click-through this session.
...
**Status:** RESOLVED — all 5 steps built, live-verified end-to-end (real deployed HTTP calls, not just direct DB/service calls), merged to `dev` (fast-forward, `aa80df2`), and the full schema migration (season column + all 5 unique indexes) mirrored to **both staging and prod**. One correction made along the way: staging's original backfill value (`'2026/2027'`) was wrong per Richard's direct correction — historical rows belong to `'2025/2026'` (the season the backfilled BUSA League data actually represents), `'2026/2027'` is the upcoming season and only correct for `CURRENT_SEASON` (future writes). Fixed on staging, applied correctly on prod from the start. See progress note below, `BACKLOG-097`/`BACKLOG-223` for step 4/middleware detail, and `BACKLOG-224` for a separate, larger data-completeness gap this surfaced (prod missing most of the historical backfill work that only ever ran against staging).
...
- ~~**Display side has the same gap, confirmed live (session 45)**~~ — **CLOSED, session 53 (2026-08-20).** `GET /api/players/[id]` now returns a new admin-only `affiliationHistory` field (all `playerTeamAffiliations` rows for the player, active and inactive, sorted newest-first) alongside the existing active-only `memberships`. `/admin/roster-transfers` renders it as a timeline (team, sport, affiliation type, season, date range, active/past badge) when a player is selected, refreshing after a successful transfer instead of resetting the form. Live-verified against the real `LIGHT` player — Vikings (active), Rim Reapers (past, closed), and the separate COLNAS college affiliation all correctly surfaced, `memberships` unaffected. **Found in the same pass, not fixed:** the Rim Reapers row's `startDate`/`createdAt` are corrupted (`+057867-...`, a nonsensical far-future year — left over from session 45's original manual fix script). `Date` parses it as technically-valid rather than `NaN`, so it renders as an absurd date instead of crashing or silently hiding — cosmetic, pre-existing, not introduced by this fix, not blocking. **Root cause confirmed later the same session** (while live-verifying the 3 remaining season-write insert sites, see `RUNLOG.md`): a raw-SQL write stored `.toISOString()` (a string) into `playerTeamAffiliations.startDate`/`endDate`, a Drizzle `integer({mode:'timestamp'})` column that actually stores epoch *seconds* — SQLite accepts the mistyped write silently, but Drizzle's later read produces this nonsensical date. Not fixed — the real intended date isn't known without checking session 45's original source data (would need Richard or that session's own notes, not something to guess/fabricate).

**Concrete build plan, session 53 (2026-08-20) — investigated in full, not yet built:**

1. **Schema — `playerTeamAffiliations`.** Add a `season` column; widen the unique index from `pta_player_team_unique (playerId, teamId)` to `(playerId, teamId, season)`. Existing rows must be backfilled to a real season value, not left `NULL` — SQLite treats `NULL` as distinct-from-itself in a unique index, so leaving legacy rows `NULL` would silently turn the constraint off for exactly the rows it's meant to protect.
2. **Schema — stats tables.** Add a real unique index `(playerId, season, competitionId)` to `basketballPlayerStats`/`footballPlayerStats` (plus `sport` for `individualSportStats`) — none exists today, confirmed via grep, nothing has ever stopped duplicate rows. Known limitation to accept, not solve here: a `NULL` `competitionId` (friendly matches) won't dedupe against other friendlies for the same player/season — real but low-priority edge case.
3. **`updatePlayerStats()` rewrite** (`src/app/api/matches/[id]/events/route.ts:481-642`) — the actual bug is worse than the hardcoded `season: '2024'` literal: the lookup is `WHERE playerId = ?` alone, completely ignoring season/competition, so every player has one lifetime-blended row regardless of season, forever. Fix: derive `season` from the match's real competition (`competitions.season`) and scope select/update/insert by `(playerId, season, competitionId)` instead of `playerId` alone.
4. **`BACKLOG-097` standings recalculation** — bundled into this plan since "a season isn't ready if the tables it produces don't update." Wire a real recalculation into the match `FINISHED` transition; keep `teams.played/won/drawn/lost` in sync from the same trigger rather than retiring it (per that entry's own audit note), since other code may depend on it.
5. **Minimal admin roster-transfer UI** — new and separate from the existing `/admin/transfers` page (confirmed session 53: that page + the `transfers` table is a transfer-*news*/rumor-announcement feature with `reliability`/`source`/push-notification fields, unrelated to the roster-affiliation mechanism this item needs — left untouched, 🔴 High Volatility). One action: pick player → pick new team → confirm; server closes the current active affiliation (`isActive=false`, `endDate=now`) and creates a new one. New admin-gated API route.
...
- **Live-verified against staging (2026-08-20, after pushing `c88e8cd` to `origin/dev`):** fired a real `GOAL` event through the deployed `/api/matches/[id]/events` route (real admin bearer token, real `busa-joga-player-1`, real `BUSA LEAGUE FOOTBALL` competition, `season='2025/2026'`). Result: the pre-existing legacy row (`season='2024'`, `competitionId=null`, `goals=0`) was left completely untouched; a brand-new, correctly-scoped row was created (`season='2025/2026'`, `competitionId='xm1OcBFeugKxLDHH6Xi6p'`, `goals=1`) — proving the season/competition scoping works end-to-end on the real deployed route, not just in theory. First verification attempt (before the fix was pushed) caught the deployed app still running the old code and incremented the legacy row instead — reverted cleanly before re-testing. Throwaway match + new stats row fully deleted after, confirmed via post-delete read.
- **Note on how `c88e8cd` reached `origin/dev`:** pushed directly (matching this session's earlier precedent for the two prior bundles) before Richard's request to use a feature branch for the rest of this initiative — steps 1-3 stay as shipped rather than rewriting shared `dev` history. `feature/season-readiness` (branched off `dev` at `c88e8cd`) now carries steps 4-5 and anything further in this initiative.
- **Step 4 (standings recalculation) built on `feature/season-readiness`** — see `BACKLOG-097` for full detail (new `standingsService.ts`, wired into the FINISHED transition, `standings` unique index added, real bug caught and fixed mid-verification, DB-level live-verified against staging).
- **Step 5 (admin roster-transfer UI) built and DB-level-verified on `feature/season-readiness`:** new `transferPlayerToTeam()` in `src/lib/rosterService.ts` (extracted from the route for direct testability, same pattern as `standingsService.ts`) — resolves the new team's real `affiliationType` (`resolveAffiliationType`, moved here from its old inline copy in `roster/route.ts`, now shared), closes the player's current active affiliation of that *same* type, opens a new one at `CURRENT_SEASON` ('2026/2027', single exported constant), or reactivates a pre-existing same-season row instead of inserting (required, since `pta_player_team_season_unique (playerId, teamId, season)` forbids a second row for the same triple — e.g. a mid-season rejoin). Wired to a new admin-gated route (`POST /api/admin/players/[playerId]/transfer`) and a minimal new admin page (`/admin/roster-transfers` — search player, pick new team, optional jersey/position, confirm; no history browser, no bulk mode, deliberately minimal per the plan). Also closed a gap the step-1 migration itself introduced: `roster/route.ts`'s two `insert(playerTeamAffiliations)` calls never set `season` at all (would have inserted `NULL`, silently opting new roster adds out of the season-scoped uniqueness this whole migration exists for) — both now set `season: CURRENT_SEASON` from the same shared constant.
  - **Live-verified against real staging DB** (`tsx`-run, same reasoning as step 4 — this branch isn't deployed yet): used a fully throwaway player (never real production roster data) on two real teams (`busa-joga`/`busa-wolves`, both genuinely owned by the same `type='university'` organization — confirmed via direct query, not assumed). Three scenarios all correct: (1) transfer A→B closed the real A affiliation (`isActive=false`, `endDate` set) and opened a new active B row with jersey/position; (2) transfer back B→A reused/reactivated the *original* A row rather than inserting a 3rd (exactly 2 rows existed throughout, confirmed via direct count); (3) attempting to "transfer" to the team already actively affiliated threw the expected `409`. Real `busa-joga-player-1`'s actual affiliations confirmed untouched via a final direct read. Throwaway player + both its affiliation rows fully deleted after.
  - **Test-setup bug caught and self-corrected, not a code bug:** first verification pass hardcoded `affiliationType: 'club'` for the throwaway player's starting affiliation instead of using the real resolved type — `resolveAffiliationType('busa-joga')` actually returns `'university'` for real BUSA teams (both are owned by Bells University of Technology's org node). The mismatch made the service correctly skip closing the "wrong-typed" row (working as designed — a same-type-only close is intentional, see the college/university-permanence comment), which looked like a bug until traced to the test's own setup. Corrected by querying the real `resolveAffiliationType()` before constructing the test's initial row.
- **Real end-to-end deployed verification, 2026-08-20 (session 53) — steps 4 and 5 both confirmed against the actual `feature/season-readiness` Vercel preview** (`https://brixsports-staging-493a4yw78-brixsports-projects.vercel.app`, not a direct service call this time): a real `PATCH /api/matches/[id]` with `status: 'FINISHED'` against a throwaway LIVE match correctly triggered `recalculateStandingsForMatch()` via its `after()` hook — `standings` went from no row to the full correct historical aggregate for both teams, matching the earlier direct-call verification exactly. A real `POST /api/admin/players/[playerId]/transfer` against a throwaway player correctly closed the old affiliation and opened the new one. Both fully cleaned up after (throwaway match, standings rows, teams reverted to zero, throwaway player deleted); real `busa-joga-player-1` confirmed untouched. **Status upgraded to fully deployed-and-verified for both steps** — see `BACKLOG-223` for a real, separate bug this same verification pass surfaced along the way.
- **Prod mirror complete (2026-08-20, session 53):** merged `feature/season-readiness` → `dev` (fast-forward, `aa80df2`), then mirrored the full schema migration to prod — `season` column + `pta_player_team_season_unique` on `playerTeamAffiliations` (296 existing rows backfilled to `'2025/2026'`, the corrected value), plus all 4 stats/standings unique indexes. Pre-flight found 0 duplicate rows against every target constraint on prod — safe to apply as-is. Independently re-verified via a fresh query after the run. Full detail in `RUNLOG.md` 2026-08-20.
- **Remaining `playerTeamAffiliations` insert-site gap closed (2026-08-20, session 53, `fix/season-followups-53` commit `bca325d`):** the 4 insert sites the original migration missed (`players/route.ts`, `players/bulk-register/route.ts`, `players/[id]/route.ts`, `competitions/register/approve/route.ts`) all now set `season` — the two competition-aware routes (`bulk-register`, `competitions/register/approve`) derive the real season from their `competitionId` (same pattern as `updatePlayerStats()`), the other two use `CURRENT_SEASON` since they have no competition context. Live-verified: a real `POST /api/players` against the deployed preview created its affiliation row with `season: '2026/2027'` (previously would have been `NULL`).
...
### BACKLOG-223 — `middleware.ts`'s Admin Route Gate Only Accepts a Cookie, Not the Bearer Token Every Admin Route Handler Itself Supports

**Status:** RESOLVED (2026-08-20, session 53) — `middleware.ts`'s admin-gate token extraction now checks `Authorization: Bearer` first, falling back to the cookie, matching `getAuthUser()`'s own precedence exactly (`fix/season-followups-53`, commit `26ef5b8`). Live-verified against the real deployed preview: the same request that previously 401'd (`GET /api/admin/teams/busa-joga/roster` with a Bearer-only admin JWT, no cookie) now returns `200`.
**Priority:** Low — no security exposure (the gate is *more* restrictive than the route handlers, not less; a legitimate cookie-bearing admin session is unaffected), but a real functional gap for any non-browser caller
**Filed:** 2026-08-20 (session 53), found while live-verifying `BACKLOG-126` step 5 against the real deployed `feature/season-readiness` preview

**Problem, confirmed live, not theoretical:** `src/middleware.ts`'s "Admin route protection (all environments)" block (lines 61-92, matcher includes `/api/admin/:path*`) reads only `request.cookies.get('authToken')` — there is no `Authorization: Bearer` fallback. Every actual `/api/admin/*` route handler calls `getAuthUser(request)` (`src/lib/auth.ts:41-49`), which explicitly supports **both** — checks the `Authorization` header for a `Bearer ` prefix first, falls back to the cookie. So the two server-side auth layers on the exact same route disagree about what a valid credential looks like: middleware runs first and rejects a Bearer-only request with a 401 before the route handler (which would have accepted it) ever runs.

**Confirmed live, not just by reading the code:** a real admin-role JWT sent as `Authorization: Bearer <token>` against `GET /api/admin/teams/busa-joga/roster` (an existing, unmodified route) got a real `401` from the deployed preview; the *identical* token sent as a `Cookie: authToken=<token>` header against the *same* route got a real `200`. Not route-specific — this is `middleware.ts`'s own matcher applying uniformly to every `/api/admin/*` path.

**Impact:** any legitimate caller that authenticates via Bearer token rather than a browser session cookie (a script, a mobile client, any future non-browser admin integration) gets a false `401` on every single `/api/admin/*` route, regardless of how valid its credentials are. The existing admin dashboard UI is unaffected in practice — browser `fetch()` calls always carry the session cookie automatically, so this has presumably never been noticed. Caught only because this session's live-verification scripts (`dev/*.mjs`, run outside a browser) use Bearer tokens throughout, the same pattern already used successfully all session against every *non*-admin route.

**Fix (not built):** either add a `Bearer` fallback to `middleware.ts`'s admin-gate cookie check (mirroring `getAuthUser()`'s own precedence), or — simpler, avoids duplicating auth logic in two places — drop the redundant admin-gate block entirely and rely on each route handler's own `getAuthUser()` + role check, which CLAUDE.md already mandates as the authoritative check ("Admin API routes must call getAuthUser(request) AND check user.role === 'admin' — never trust middleware alone"). The middleware layer would then only need the staging-wide gate (lines 41-59), not a second, narrower, inconsistent admin-specific one.

**Not fixed this session** — out of scope for `BACKLOG-126`, flagged rather than silently patched mid-feature.
...
**Status:** SUPERSEDED/RESOLVED — stale entry correction, session 53 continuation, 2026-08-22, found while triaging for genuine open live-blockers. This was filed 2026-06-19 (session ~19) and never revisited even though its exact complaint — no server-side WS emit on event save — was the direct subject of `BUG-108`/`BUG-116` (session 44, root-caused to `NEXT_PUBLIC_WS_URL` missing its `https://` scheme, live-verified end-to-end), `BUG-119` (broadcast latency, 42s→~9.9s), and `BUG-153` (period/status broadcast wiring). The Railway-WS-is-dead premise this entry was built on no longer holds. Left struck-through per this project's own convention rather than deleted, so the history isn't lost.
**Priority:** was HIGH — Flow B and Flow C correctness
**Filed:** 2026-06-19
...
**Status:** RESOLVED on `feature/season-readiness` (not yet merged to `dev`/mirrored to prod) — live-verified twice: first directly against the staging DB (tsx-run), then end-to-end through a real deployed `PATCH /api/matches/[id]` against the actual `feature/season-readiness` Vercel preview (2026-08-20), confirming the full pipeline (route → `after()` → `recalculateStandingsForMatch()` → DB) works exactly as designed with no gaps between the direct-call test and real HTTP behavior.
...
**Built (2026-08-20, session 53, `feature/season-readiness`):** `src/lib/standingsService.ts`, new file — `recalculateStandingsForMatch(matchId)` recomputes both `standings` (competition-scoped, using each competition's real `pointsForWin`/`pointsForDraw` from `competitionSportSettings`, default 3/1/0 if unset) and `teams.played/won/drawn/lost/goalsFor/goalsAgainst/points` (team-level, aggregated across ALL of a team's FINISHED matches regardless of competition, since `teams` has no `competitionId` column — `points` on this table deliberately uses the fixed 3/1/0 default rather than each match's real per-competition rule, since mixing different competitions' point systems into one number isn't well-defined; documented in-file, `standings.points` is the authoritative per-competition number). Fully recomputed from FINISHED matches each call, not incremental — safe to call more than once for the same match. Wired into `src/app/api/matches/[id]/route.ts`'s PATCH handler via `after()`, firing only when `body.status === 'FINISHED'`. Also added `standings_team_competition_unique (teamId, competitionId)` — `standings` had **zero** real unique index before this (confirmed via `sqlite_master`, only the PK autoindex existed), meaning `POST /api/standings`'s own pre-existing `onConflictDoUpdate({ target: [teamId, competitionId] })` would have thrown at runtime the moment it hit a real conflict — a separate, real latent bug this closes as a side effect. 0 existing duplicate `(teamId, competitionId)` rows confirmed before adding the constraint (26 total rows, 22 with `competitionId=null` — same accepted NULL-doesn't-dedupe limitation as the season-readiness stats indexes).
**Bug caught mid-verification, fixed same session:** first implementation of `aggregateTeamRecord()` used a single `competitionId: string | null` parameter for two different meanings — "scope to this specific competition" vs. "no competition filter at all" (needed for the `teams`-level aggregate) — both collapsed to the same `isNull(matches.competitionId)` SQL branch when passed `null`, so the teams-sync silently matched zero rows (every real match has a real `competitionId`) and overwrote `teams` with all-zero records instead of the real aggregate. Caught because live verification checked actual before/after values instead of just "did it run without throwing." Fixed with an explicit `'all'` sentinel distinct from `null`.
**Live-verified against real staging DB** (`busa-joga`/`busa-wolves`, real `BUSA LEAGUE FOOTBALL` competition — this branch isn't deployed anywhere yet, so verified via `tsx`-run of the real service function directly against the staging DB, same precedent as `dev/debug-phase1-trigger.mjs`, not yet through an actual deployed HTTP PATCH call): created a throwaway FINISHED 3-1 match, called `recalculateStandingsForMatch()`, confirmed both `standings` and `teams` picked up the full real historical FINISHED-match record for both teams (not just the throwaway) with correct W/D/L/GF/GA/points math (`busa-joga`: 6W-1D-0L → 19 pts at 3/1/0; `busa-wolves`: 1W-3L → 3 pts). Throwaway match deleted and both tables reverted to their exact pre-test snapshot (empty standings rows, all-zero teams rows) after, confirmed via post-cleanup read. Full detail in `RUNLOG.md` 2026-08-20 (session 53).
**Not yet done:** a real end-to-end test through the actual deployed PATCH route (requires this branch to reach a deployed environment — Vercel preview via a PR, or a merge to `dev`), prod mirror of the `standings` unique index migration.
...
**Fourth bug found session 50, more severe than the three above: `push_subscription_matches` itself was never migrated to prod.** All of the above was verified staging-only. A read-only check against prod this session (`dev/check-push-tables-prod.mjs`) found `push_subscriptions` and `match_reminders` present on prod, but `push_subscription_matches` genuinely `MISSING` — meaning the entire anonymous/authenticated per-match Bell subscribe flow has been non-functional on prod (any insert attempt would 500 against a nonexistent table) since this feature merged, despite the feature itself shipping to prod in the code. Fixed same session: pulled the real `CREATE TABLE` DDL from staging's own `sqlite_master` (not reconstructed from `schema.ts`) and created the identical table on prod (`dev/migrate-push-subscription-matches-prod.mjs`), host-fingerprint-guarded before writing. Verified via `PRAGMA table_info` — columns match staging exactly. Logged in `RUNLOG.md`. Caught because Richard asked directly whether the last migration had been mirrored to prod — worth treating "did the schema actually ship, not just the code" as a standing checklist item for any future DB-touching feature, not just this one.
...
`resolveConflict` now does a real server-side write for `keep-first`/`keep-second` (the only two resolutions `MultiLoggerStatus.tsx` actually renders a button for — `keep-both`/`merge` never called from the UI) — calls the existing, already-atomic `DELETE /api/matches/[id]/events/[eventId]` route on the losing event, which already handles score/stat revert and its own WS broadcast. Previously this only flipped a local `resolved: true` React flag; the DB and every public viewer kept the conflicting data untouched.

**Documented, honest scope limit, not silently dropped:** conflict *detection* itself still only runs on the poll cycle (`detectConflicts` needs the full local event set, which the hook doesn't hold on a single incoming push) — this fix closes "the broadcast never leaves the tab," not "a conflict is flagged instantly." Noted in the hook's own code comments.

**What's still required before RESOLVED:** a real two-device dual-logger test on staging — two real logger sessions on the same live match, confirming (a) an event logged by logger A appears for logger B without waiting ~10-15s, (b) a real conflict (e.g. both log a goal for the same player within the 5s window) shows the resolve banner and `keep-first`/`keep-second` genuinely deletes the losing DB row (not just clears the banner) — per `.agents/rules/backlog.md`'s evidence standard, "the UI showed X" and "no errors in console" do not count; needs an actual DB query confirming the losing event row is gone and the score/stat reverted correctly. This is also CLAUDE.md's own still-open Live Event Readiness Checklist item.

**Live-test attempt #1, session 53 continuation, 2026-08-22 — partial, inconclusive, root cause found:** set up two real, isolated logger sessions (Test Logger + Emmy, both real DB rows) concurrently on a real staging match against `brixsports-staging.vercel.app`. Presence/join worked ("2 loggers active" in real time), but the branch was discovered mid-test to have never been pushed to `origin` — `git ls-remote --heads origin` had no match, `origin/dev`'s `useMultiLogger.ts` had zero occurrences of `logger:broadcast-event`. Staging was running pre-fix code the entire time; the broadcast-timing test wasn't reached before this was found. Branch pushed and PR #22 opened (`Brixsport/BrixSports`) immediately after.

**Live-test attempt #2, same session, against PR #22's actual preview deployment (has the real fix, confirmed same staging DB/match data):** two isolated Playwright contexts (Test Logger + Emmy) on `F-Oqtj3HKpWjBc35R8k89`. Results:
- **Presence/join: confirmed working**, same as attempt #1 — `MultiLoggerStatus` showed "2 loggers active" in real time on both sides.
- **Event creation + score update: confirmed fast and correct.** Test Logger logged a real Goal (scorer: Effiong Uduak) + Assist (ALI) — DB-confirmed via direct query: `match_events` got both rows at `created_at` 10:09:59.000Z, ~1.1s after the click sequence completed. `matches.home_score` correctly flipped `0→1` (`away_score` stayed 0) — real DB read, not UI-inferred.
- **Broadcast arrival on the OTHER logger's screen: NOT confirmed within a 10s observation window.** Emmy's screen still showed 0 EVTS / 0-0 / "WAITING FOR EVENTS" when the poll loop gave up. Since the DB write itself completed in ~1.1s, a 10s window should have been more than enough for a real-time WS push to land — this specific gap (event saves correctly, but the *push to the other connected logger's screen* wasn't observed) is the one piece of BACKLOG-151 still not proven working. Could not re-check whether it eventually arrived via the poll fallback (~10-15s) since the isolated browser context had already closed by the time this was noticed.

**Net assessment:** 2 of 3 pieces solid (real-time presence, and correct fast event/score persistence — the latter was arguably already true pre-fix via the REST route, so isn't new evidence for *this* fix specifically). The one piece that would actually distinguish "the WS push works" from "still falling back to the old poll" — an event appearing on another logger's screen well under 10s — was not confirmed either way.

**Live-test attempt #3, same session, 30s window, with raw WS frame + `pageerror` instrumentation — inconclusive for a new reason, test-environment artifact identified, not an app-code finding:** re-ran with a longer 30s poll and, critically, attached `page.on('websocket', ws => ws.on('framereceived'/'framesent'))` to see the actual Socket.IO wire traffic rather than inferring from DOM state alone. Result: the goal+assist DID save correctly (DB-confirmed again, home_score `1→2`, event count `2→4`), Test Logger's `logger:broadcast-event` frame genuinely left the client (`WS frame OUT` captured with full payload) — but Emmy's screen never updated in 30s, same as attempt #2. The frame capture explains why: **both sockets were flooded continuously (multiple times per second) with `{"message":"Unauthorized: logger authentication required","type":"auth:required"}` error frames from the WS server**, meaning `socket.data.isLogger` was `false` on both connections the whole time — every `requireLogger`-gated action (`logger:join`, `logger:broadcast-event`, `match:time:update`, etc.) was silently rejected server-side. The earlier "2 loggers active" confirmation in attempts #1/#2 was almost certainly coming from the REST fallback path (`joinMatch()`'s `POST /api/matches/[id]/loggers` + `pollActiveLoggers()`), not the new WS channel at all — meaning the WS half of this fix has never actually been exercised successfully in any live test run yet.

**Root cause traced, and it's the test harness, not the app:** `ws-server/index.js`'s `getEnvFromOrigin()` classifies a connection as `staging` only if the page's Origin contains the literal substring `brixsports-staging.vercel.app`; anything else (including `prod`) falls through to `JWT_SECRET_PROD`. This test ran against PR #22's **branch-alias preview URL** (`brixsports-staging-git-feature-backl-c19232-brixsports-projects.vercel.app`) — which does *not* contain that exact substring — so both sockets got classified as `prod` and verified against `JWT_SECRET_PROD`, while the test's hand-rolled JWTs were signed with `.env.local`'s `JWT_SECRET` (the staging value). Secret mismatch → `jwt.verify` throws → `isLogger=false` → every logger-gated WS emit rejected, matching the observed flood exactly. **This means BACKLOG-151's WS mechanism is still not actually proven working or broken** — the real test needs to run against an origin the WS server actually classifies as `staging` (the real `brixsports-staging.vercel.app` domain once merged, or a origin-detection tweak for PR-preview aliases if testing pre-merge is required again).
...
1. ~~**Campaign composer scope decision**~~ — **RESOLVED session 50.** Richard's call: bring it in scope, harden now. `CLAUDE.md`'s "Explicit Out of Scope" list no longer excludes push campaigns; `Scope Boundaries` now explicitly includes match/team push notifications (event-triggered, reminders, and the admin composer).
2. ~~**Campaign composer hardening**~~ — **RESOLVED session 50.** `send/route.ts`'s `getTeamFollowerUserIds()` now filters out `userPreferences.matchAlerts === false` the same way `sendMatchEventNotification()` does. `targetAudience: 'all'` now excludes `BACKLOG-150`'s anonymous per-match subscribers (`user_id = 'anonymous-push-subscriber'`) — they consented to one match, not a general broadcast. Send-history was already fixed as a side effect of `BACKLOG-211` (direct write to `notification_send_log` replacing the broken self-`fetch()`).
...
6. **Overview-tab empty-state redesign** — separate from the livestream embed itself (which is already fully built and just needs an admin to populate one match's fields to verify end-to-end). The "lots of unused white/black space" complaint is actually the placeholder shown for the ~100% of matches with no livestream configured — a real UI design task, independent of the livestream feature. **Still open, deliberately not built session 51** — this is a genuine design decision (what should fill the space: recent form, H2H preview, key stats, something else), not a quick fix, and Richard's already planned a cross-platform (SofaScore etc.) UI research pass for the upcoming UI-refinement sprint — better decided there than freelanced here.
7. ~~**Livestream embed source allowlist**~~ — **RESOLVED session 51 (2026-08-11).** `LivestreamPlayer` renders 'custom'/'hls'/'dash' stream URLs directly as an iframe src with no host check (only http/https scheme was validated) — 'youtube'/'twitch'/'facebook' were never actually exposed to this since they only ever extract an ID/channel against a fixed trusted domain. New `src/lib/livestream-allowlist.ts` (`ALLOWED_LIVESTREAM_EMBED_HOSTS` + `isAllowedLivestreamEmbedHost()`) gates both the admin-only `PATCH /api/matches/[id]/livestream` route (falls back to the row's existing `livestreamType` when a raw API call updates the URL without also sending the type, closing a bypass the naive version would have had) and `LivestreamPlayer.tsx` itself as defense-in-depth. Starting allowlist: youtube/twitch/facebook's own domains (for direct links pasted under 'custom'), vimeo, Cloudflare Stream, Mux, and Cloudinary (already a project dependency). **Evidence:** `tsc --noEmit` clean (49 baseline, none new); allowlist logic unit-verified against 7 cases including the classic subdomain-spoof bypass (`vimeo.com.evil.com` correctly rejected, `sub.vimeo.com` correctly allowed) — all passed. Not live-verified through the actual admin UI + PATCH request this pass (would need an admin session; the logic itself is fully covered by the unit cases). Write path was already admin-only (`getAuthUser` + role check), so this is defense-in-depth against a careless/compromised admin pasting an untrusted URL, not a new exposure being closed against unauthenticated callers.
...
**Built (Phase 2 pilot — `profile/settings/page.tsx` retrofit):** the exact page the toggle lives on was picked as the first retrofit target, both to prove the pattern before rolling it out and to give the toggle an immediate, visible feedback loop. Mechanical rule applied: `text-white`(`/NN`) → `text-foreground`(`/NN`) (opacity suffix preserved, so the designer's original relative hierarchy between the `/40`/`/60`/`/80` labels survives exactly); `bg-white/5`(+`border-white/10`) → `bg-muted`(+`border-border`) for every card/input/button surface (the original design already used the identical value for both card containers and form controls, so no new distinction was invented); `bg-[#050505]` → `bg-background` (page root) or `bg-card` (the password-change modal, a genuinely distinct elevated surface); `text-black` on `bg-primary` → `text-primary-foreground` (was hardcoded assuming a specific primary color; the semantic token stays correct in both themes' actual primary values). Left alone deliberately: the modal backdrop scrim (`bg-black/80`, a universal dark overlay convention regardless of theme) and the toggle switch's knob (`bg-white`, adequate contrast in both themes as a small shadowed circle).

**Evidence (Phase 2 pilot):**
- `tsc --noEmit`: 49 baseline, none new.
- Grep-confirmed after the retrofit: zero remaining `bg-white`/`text-white`/`border-white`/`bg-[#...]`/`text-black` in the file except the two deliberate exceptions above.
- **Live-verified against the real deployed `brixsports-staging.vercel.app` (dev branch pushed, `e7a160b`), not just local computed styles.** Dark mode: card visibly lighter than the page (`bg-muted` vs `bg-background`), correct text hierarchy, blue-tinted icon circle. Clicked the real "Light" button via the actual DOM: whole page flipped correctly — light gray page, near-white/light-gray cards distinguishable by border, dark text, inputs and `<select>`s correctly `bg-muted`/`border-border`, toggle switches' primary-blue "on" state and white knob both read correctly against the light background, icon tint stayed legible. Confirmed via `getComputedStyle` on both account inputs post-fix: `background-color: oklch(0.95 0 0)` (exactly `--muted`), `color: oklch(0.15 0 0)` (exactly `--foreground`) — matches the CSS, not just eyeballed. One transient black flash on the "Full Name" input during the exact click transition turned out to be a focus/render artifact, not a real bug — a clean page reload in the same (light) state renders it correctly every time.
- **Known limitation of this verification, not a product bug:** the injected test session didn't reach a fully authenticated `useAuth()` state (footer still showed "Sign In"), so `loadSettings()`'s DB-preference fetch never ran and the `ThemeButton`'s own `active` highlight (driven by local `settings.theme` state, separate from the real applied theme) stayed visually stuck on "Dark" even after Light was clicked and correctly applied everywhere else on the page. The actual re-theming — the thing this item is about — worked correctly regardless; a real login would keep the button highlight in sync too, untested here for lack of a real session.

**Real contrast bug found via a genuine authenticated screenshot from Richard (real login, real Chrome, staging), fixed same session:** the disabled Email Address input, and separately the Save/Update-Password buttons' disabled state, all used a bare `opacity-50` on the whole element. `opacity-50` fades everything inside an element — text included — toward whatever's *behind* it, not toward a fixed target color. That was tuned by eye against this design's near-black dark-mode backdrop, where a 50%-faded near-white text still reads as a legible medium gray. Against light mode's near-white backdrop, the identical class fades already-dark text toward near-white too, washing it out close to unreadable — confirmed directly in Richard's screenshot (the Email field's placeholder-like faint gray-on-light-gray). Root cause generalizes: any bare `opacity-NN` applied to a text-bearing element is backdrop-relative, not theme-safe, regardless of which specific colors are inside it. Fixed by never fading text via a wrapping `opacity`: the disabled input now uses explicit `text-muted-foreground` (a fixed, contrast-checked color in both themes) with only the background faded (`bg-muted/60`, which doesn't carry text through it); the two primary-colored disabled buttons now fade `bg-primary/60` only, leaving `text-primary-foreground` at full opacity so the label stays legible against the lightened button in both themes.

**Evidence (contrast fix):**
- `tsc --noEmit`: 49 baseline, none new.
- Root cause and fix are structural (removing the specific mechanism that produced the bug), not tuned by eye — a bare `opacity-NN` is not reintroduced anywhere in this file (`grep -n "opacity-" src/app/profile/settings/page.tsx` shows zero remaining bare instances after the fix, only the two `bg-primary/60`/`bg-muted/60` target-color fades).
- Not yet re-confirmed with a fresh screenshot in light mode post-fix (requires a real authenticated session, same limitation noted above) — will confirm on the next real login screenshot.
...
**Status:** SHIPPED — 2026-08-11 (session 51), not yet live-tested
...
**Fix:** distinguished the two cases explicitly in both `checkAuth()` and `refreshSession()` (which had the identical collapsed-failure pattern, not just the one function originally filed). A caught exception (network/fetch failure) no longer clears `user` state or the stored token — auth state is left as-is, resolved by the next periodic check or an explicit user action. Only an actual `401`/`403` HTTP response is treated as "genuinely logged out" (clears `user` + removes the token). Any other non-2xx status (`5xx`, gateway timeout, etc.) now logs a warning and leaves auth state untouched, same as the network-failure case, instead of being lumped in with a real `401`.

**Evidence:**
- `tsc --noEmit`: 49 baseline, none new.
- Not live-tested this session — local dev server was unstable/unresponsive for repeated stretches (see session's own process notes), and a genuine network-failure condition is inherently hard to simulate cleanly against a real server in this environment. Confidence is code-level: the exact branch logic was traced line-by-line against the original bug description, both affected functions (`checkAuth` and `refreshSession`) got the same fix shape, and `tsc` confirms no type errors introduced.
- Pending items: a real live test (e.g. throttling/dropping the network mid-session, or forcing a 5xx from `/api/auth/me`) to confirm the auth state genuinely survives.

**Found:** session 51 (2026-08-11), Richard asked directly whether a bad connection would show the app as anonymous — confirmed via direct code read of `checkAuth()`, not inferred. **Fixed:** session 51, same session.

---

### BUG-218 — `profile/settings/page.tsx`'s Notification Toggles Don't Control Real Notification Sends (Field Mismatch with `SettingsOverlay.tsx`)

**Status:** RESOLVED — 2026-08-11 (session 51)
**Priority:** High — genuine user-facing correctness bug: a user can turn a toggle off on this page and keep receiving exactly the notifications they thought they'd disabled

**Problem:** found via a full settings-page functional-wiring audit (background agent, session 51). Two separate, disconnected UIs both claim to control "match notifications": `profile/settings/page.tsx` (this page) wrote `notifications`, `emailNotifications`, `matchReminders`, `favoriteTeamUpdates`, `weeklyDigest` to `userPreferences` — but the real send paths, `src/lib/notifications/match-notification-service.ts` and `src/app/api/notifications/send/route.ts`, only ever checked `userPreferences.matchAlerts`, a column this page never touched at all. That field is instead controlled by a completely separate component, `src/components/SettingsOverlay.tsx` (a different modal, not this page). Traced further: `userPreferences.matchReminders` (the boolean this page's "Match Reminders" toggle wrote) is not read by anything, anywhere — including a name collision with an entirely unrelated table also called `matchReminders` (the real per-match, explicit-opt-in reminder system, `src/app/api/reminders/route.ts`, keyed by an actual row per match+user, no global boolean gate at all). `emailNotifications`/`favoriteTeamUpdates`/`weeklyDigest` are also write-only with zero consumers anywhere — no email pipeline, no per-team filtering, no digest feature exist in the codebase to back them. The generic `notifications` boolean (behind the "Push Notifications" label) *did* correctly round-trip to/from the DB (contrary to the audit's initial read, which mistook a field-name remap in the save handler — `notifications: settings.pushNotifications` — for a dropped write) but was never read by any send path either, so toggling it had zero real effect either way.

**Fix:**
- `src/lib/notifications/match-notification-service.ts` and `src/app/api/notifications/send/route.ts` — the subscriber-exclusion query in both now excludes a user if **either** `matchAlerts = false` **or** `notifications = false` (an OR, not replacing one with the other) — makes "Push Notifications" a genuine master mute on top of the existing granular `matchAlerts` gate, rather than doing nothing.
- `src/app/profile/settings/page.tsx` — dropped the orphaned `matchReminders` state entirely; added a real `matchAlerts` field (load from `prefsData.preferences.matchAlerts`, save it in the PATCH body — the API route already accepted this field, since `SettingsOverlay.tsx` was already using it). Relabeled the UI row from "Match Reminders" to "Match Event Alerts" to honestly describe what it now actually controls (goal/card/etc. push, not the separate per-match reminder system).
- **Follow-up, same session:** Richard's call on the open decision above — hide, don't build. `emailNotifications`/`favoriteTeamUpdates`/`weeklyDigest` UI rows in `profile/settings/page.tsx` are now `{/* BACKSCOPED session 51: ... */}` (matching this project's existing backscope convention, e.g. `BACKLOG-028`'s Predictions/Polls tabs) — commented out, not deleted, so reinstating is a straightforward uncomment once a backing system exists for any of them. State/load/save for these three fields deliberately left intact (not gutted) so the round-trip still works if the UI comes back.

**Evidence:**
- `tsc --noEmit`: 49 baseline, none new.
- **Real DB-level test against staging, not a simulated read.** The one real user in staging with an actual push subscription (`DHjNopjm1ygPRvZEXB0H9`) was used for a controlled, restore-guaranteed test (`dev/verify-notif-master-mute-51.mjs`): flipped `notifications` to `false`, ran the exact new OR-exclusion query against real data → correctly returned this user as excluded (1 row). Original value restored immediately after in a `finally` block, confirmed via a follow-up read (`match_alerts: 1, notifications: 1`, unchanged).
- Grep-confirmed zero remaining references to the old `settings.matchReminders`/`updateSetting('matchReminders', ...)` anywhere in the file.
- UI relabel confirmed live via `get_page_text` against a running local dev server: "Match Event Alerts" renders in place of the old "Match Reminders" row.
- A full authenticated click-through (toggle it off via the real UI, click Save, confirm the DB write) was attempted but blocked by a `jwt malformed` error on the injected test token in this environment — not investigated further given the DB-level and code-level evidence already independently confirm both the query logic and the save-path wiring are correct.
- Pending items: a real authenticated UI click-through; the emailNotifications/favoriteTeamUpdates/weeklyDigest decision (build vs. hide).
...
3. **The URL-field-placement answer**: the field isn't missing, it's just on a different page than you'd naturally expect if you're already in the match-creation modal. `/admin/livestreams` **is** linked in the admin sidebar nav (`AdminSidebar.tsx:24`, admin-only), so it's not orphaned/undiscoverable — but the match modal itself had zero link or hint pointing there, so an admin creating a match had no in-context nudge that livestream setup is a separate step. **Resolved, same session, Richard's call (option b):** added a "Manage Livestream" icon-button to each `UPCOMING`/`LIVE` match row in `/admin/matches` (`src/app/admin/matches/page.tsx`, next to the existing Manage Lineups/Adjust Ratings/View/Edit/Delete row actions), linking to `/admin/livestreams?matchId=<id>`. `/admin/livestreams/page.tsx` gained `useSearchParams()` support (wrapped in `Suspense`, matching this codebase's existing `reset-password/page.tsx` pattern) — reading a `matchId` query param and auto-opening that match's edit form via the page's existing `handleEdit()`, instead of leaving the admin to find it again in that page's own separate list. Guarded to only match the page's own `UPCOMING`/`LIVE` filter (a `FINISHED` match's deep link would silently find nothing, since that page never lists finished matches — matches existing scope, not expanded). `tsc --noEmit`: 48 errors, unchanged, zero new in either touched file.
...
### BUG-235 — CRITICAL x2: Unauthenticated Admin WS Channels + Silent Stat-Loss Race in `updatePlayerStats()`

**Status:** RESOLVED — 2026-08-21 (session 53), found by a cross-session `feature`+`code-reviewer` sweep of Season Readiness / Realtime / Tier 0 (per `SYSTEM_CRITICALITY_MAP.md`'s own definition: the real-time match infrastructure — logger→DB→WS→viewer, live score accuracy/latency, clock, WS resilience, match status/period lifecycle — "any open bug or gap in this tier outranks any item in any other tier").
**Priority:** CRITICAL x2 — fixed immediately per CLAUDE.md's own rule that CRITICAL bugs don't survive in the backlog.

**Finding 1 — unauthenticated admin WS channels, real production server:** `ws-server/index.js`'s `admin:subscribe`/`admin:livestream:subscribe`/`admin:infrastructure:subscribe` had zero role check before `socket.join(...)` — any anonymous viewer socket (viewers connect with no token, by design) could join and receive `logger:updated` broadcasts (which include `loggerId`, a field CLAUDE.md explicitly bans from any public response) plus real server uptime/connection-count telemetry. The existing `requireLogger()` wrapper deliberately doesn't apply to these (by design, for read-only viewer subscribes) — these three needed a stricter `requireAdmin()` gate that didn't exist. `server.js` (local/self-hosted dev path, not used in production — Railway's `ws-server/index.js` is) had the identical gap but worse: zero JWT verification of any kind anywhere in the file.

**Fix 1:** `ws-server/index.js`'s `io.use()` middleware now also sets `socket.data.isAdmin = payload.role === 'admin'`; added a `requireAdmin()` wrapper (mirroring the existing `requireLogger()`) and applied it to all three admin-channel subscribe handlers. `server.js` got a minimal parity fix — added a JWT-verification `io.use()` middleware (single `JWT_SECRET`, not the staging/prod split, since this file never serves both environments from one process the way Railway's does) and the same `requireAdmin()` gate on its two admin-channel handlers. Confirmed safe for real admins: the client already sends `auth: (cb) => cb({ token: localStorage.getItem('authToken') })` at handshake (`src/hooks/useWebSocket.tsx:102`), and both admin pages that call these (`admin/loggers/page.tsx`, `admin/infrastructure/page.tsx`) only ever load with a real admin JWT already in `localStorage.authToken` — no client-side change needed.

**Finding 2 — silent, unrecoverable stat loss under concurrent writes:** `updatePlayerStats()` (`src/app/api/matches/[id]/events/route.ts`) used a select-then-insert-or-update pattern that isn't atomic. `BACKLOG-126`'s new unique indexes (`fps_player_season_competition_unique`/`bps_player_season_competition_unique`, both `(playerId, season, competitionId)`) mean two near-simultaneous events for the same player/season/competition — the exact untested dual-logger case `BACKLOG-151` flags — could both `SELECT` a null row, both attempt `INSERT`, and the second throw a unique-constraint violation, caught with `console.error` only and an explicit "Don't throw" comment. The event still logged, but that player's stat increment for it was permanently and invisibly dropped, with zero Sentry capture anywhere in this function (project-wide, `Sentry.captureException` was only called in 2 files before this session's `BUG-230`/`BUG-228` work) — directly contradicting CLAUDE.md's "unhandled errors must propagate, not be swallowed."

**Fix 2:** rewrote `updatePlayerStats()` around a new `upsertPlayerStats()` helper that computes plain per-event deltas (e.g. `{ goals: 1, shotsOnTarget: 1 }`) instead of absolute new values computed off a stale `SELECT`, then does a real atomic DB-level upsert: `db.insert(table).values(...).onConflictDoUpdate({ target: [playerId, season, competitionId], set: { column: sql\`${column} + ${delta}\` } })` — a single atomic statement, closing the race for every match tied to a real competition. **One caveat, documented in the code itself and here rather than silently left**: SQLite/libsql does not treat two `NULL` `competitionId` rows as conflicting under a `UNIQUE` index (each `NULL` is distinct from every other `NULL`), so a friendly/untracked-competition match (`competitionId === null`) can't rely on the same conflict-target mechanism — for that narrower case, added an insert-first/recover-via-update-on-any-failure fallback, which turns "silently gone" into "recovered on retry" rather than fully closing the race, since a true fix there would need an external lock (out of scope). Also added `Sentry.captureException(error, { tags: { area: 'player-stats' }, extra: { playerId, eventType, sport, competitionId } })` to the outer catch, so any future failure here is visible instead of console-only.

**Evidence:** `tsc --noEmit` — 47 errors, unchanged from baseline, zero new. `node --check` clean on both edited `.js` files (`server.js`, `ws-server/index.js`). Not live-load-tested against a real concurrent dual-logger burst (would need the same kind of throwaway multi-connection test `BACKLOG-151` itself has never had run) — the atomicity of the `onConflictDoUpdate` fix is guaranteed by SQLite's own conflict resolution semantics, not by this session's testing, but a real concurrent-write confirmation remains open, same caveat as `BACKLOG-151`.
...
**Evidence:** `tsc --noEmit` — 47 errors, unchanged, zero new. Live-verified against real deployed staging (`c822886`): throwaway user with **only** a competition-follow row (`followType='competition'`, real `BUSA LEAGUE FOOTBALL` id) — no team-follow, no favorite team, nothing else that could target them — plus a fake-but-valid-shaped push subscription. Threw away a real `LIVE` match under that same competition, fired a real `POST /api/matches/[id]/events` (`Goal`) through the deployed API. `notification_send_log` confirmed: `total_subscriptions: 4`, `sent_count: 3`, and the one recorded failure was explicitly this throwaway user's own subscription (`"Subscription item2-follow-verify-sub-53: The subscription p256dh value should be 65 bytes long"` — expected, since the p256dh was a placeholder string, not real crypto). That failure message is itself the proof: the user was correctly *included* in the target set purely by their competition follow, and only failed at the delivery step for an unrelated, expected reason. Full cleanup after, confirmed via post-delete `COUNT(*) = 0`.
...
**Test-case caveat, Richard's own correction:** livestreams are only a real product concern for *upcoming* matches — nobody manages a stream URL for a match that already ended, so testing against a FINISHED match wasn't a realistic scenario. **This does not make the underlying bug status-dependent, though** — the route has no `status` filter applied to this query at all, purely `.limit(50)` ordered by `createdAt DESC`. A genuinely upcoming, not-yet-started match would hit the exact same silent failure if it simply happens to be older (by `createdAt`, not `startTime`) than the 50 most-recently-*created* rows in the table — plausible any time a batch of new matches gets created after an older upcoming one already exists. The bug is real regardless of match status; the specific match picked for this test just wasn't the scenario that matters in practice.

**Impact:** any admin trying to manage a livestream for a match outside that 50-row window via the deep-link button gets a page that just loads normally with no error, no "match not found," nothing — they'd have to know to scroll/search the livestreams page's own list manually (also capped at the same 50) instead. Silent failure, not a crash — the worse kind for a support/triage perspective, since there's no error to report.
...
**Separate finding, second re-test run against a real upcoming match:** hit a different failure entirely — the page hung indefinitely on its loading spinner, never rendering. Root cause confirmed: the served HTML referenced a JS chunk (`main-app-1c2503bdc1372daf.js`) that 404's with the wrong MIME type (`text/plain`), tripping the browser's strict MIME check and blocking hydration — a fresh reload moments later got a *different*, currently-valid chunk hash, confirming this was stale CDN-cached HTML from before a recent staging deploy, not a persistent bug. Same failure class as `BUG-026` (stale HTML after deploy → missing JS chunk) but at Vercel's edge-cache layer, not the service-worker layer `BUG-026`'s fix controls — that fix doesn't reach this. Self-resolving, narrow-window, not filed as its own numbered bug — noted here as a real risk class worth remembering if a similar hang gets reported right after a future deploy.
...
**Status:** RESOLVED — 2026-08-22 (session 53 continuation).

**Evidence:**
- Commit: `b303b9d` on `feature/backlog-151-multilogger-realtime` (pushed to origin as of this session; PR #22, https://github.com/Brixsport/BrixSports/pull/22)
...
**Fix, what actually shipped (revised — first attempt reverted, see below):** `src/components/pwa/IOSInstallPrompt.tsx` docked the card at `fixed bottom-20` unconditionally, directly on top of both admin and logger surfaces' own bottom-docked primary actions (form submit buttons, clock controls). Rather than removing the prompt for admin/logger (Richard's explicit call — an admin/logger should still be able to see and use this nudge, not lose it outright), the component now docks `top-20` instead of `bottom-20` specifically when `appType === 'admin'` (covers both `admin/layout.tsx` and `logger/layout.tsx`, which both already pass `appType="admin"`) — no known top-docked persistent header exists on either surface to collide with instead (checked). Viewer's own prompt keeps its original bottom position, unchanged. `showInstallPrompt` stays `true` everywhere it already was.

**First attempt (reverted, not shipped):** initially set `showInstallPrompt={false}` on `admin/layout.tsx` — removes the collision by removing the feature. Richard pushed back directly ("in real world it has to come up now??") — correct call: admins are real potential PWA users too, and the actual bug is the collision, not the nudge's existence. Reverted before commit; the repositioning fix above is what shipped instead.

**Known, deliberately not fixed in this pass:** `src/app/logger/layout.tsx` has the *identical* `showInstallPrompt={true}` setup (same `appType="admin"`, same `swPath="/sw-admin.js"`) — meaning a logger's live-logging screen (Flow B) is exposed to the same class of overlay-blocks-a-button risk, unconfirmed but plausible given the same 30s timer and same fixed-bottom positioning. Left alone here because: (a) it's unconfirmed — no live-logging-screen repro exists yet, only the admin Create Match one; (b) unlike the admin dashboard, loggers genuinely benefit from installing the PWA (offline queue, push notifications during live matches — a real documented feature, not just visual clutter), so blanket-disabling it there would trade away a real benefit for an unconfirmed problem. Flagged for the next beta-tester pass to check specifically, not assumed safe.
...
**Root-caused via dedicated background audit (session 55):** `/offline` is precached as a full HTML document at service-worker install time in both `public/sw-user.js` and `public/sw-admin.js` (`STATIC_ASSETS`, `cache.addAll(...)`). That precached response is a byte-exact snapshot of whatever build was live when the SW last ran `install` — including its `<script src=".../chunks/app/offline/page-<hash>.js">` reference. `CACHE_VERSION` (`'brixsport-user-v2'` / `'brixsport-admin-v2'`) was a hand-maintained literal with no tie to the build — an ordinary deploy that changes `/offline`'s compiled chunk hash does NOT change the SW script's own bytes, so the browser never re-runs `install`, and the stale precached document (with its now-404ing chunk reference) survives indefinitely across however many deploys happened since the version was last hand-bumped. Same failure class as the already-documented `BUG-026` (SW serving stale HTML with old JS chunk hashes), recurring here specifically on the `/offline` fallback, which BUG-026's original fix didn't reach. Confirmed NOT an HTTP-caching regression — `next.config.ts` still correctly sets `no-store, max-age=0` on the SW script files themselves; the bug is entirely in the precached *document inside Cache Storage*, one layer deeper than BUG-026's original fix.
...
### Session 55 tsc Sweep — 16 Non-`src/db` Errors Closed, One Real Bug Found

**Status:** RESOLVED — session 55, 2026-08-25.
**Evidence:**
- Commit: not yet committed (Richard's explicit call this session — code changes present in working tree, not yet staged).
- Verified by: `npx tsc --noEmit`, full-project run before and after. Before: 76 total errors (16 in live non-`src/db` app code + 7 in confirmed-dead `src/app/api/squads/*` + 53 in `src/db/*` scripts across 5 files — recount at closure time showed 11 distinct `src/db` error lines, some multi-line overload messages). After: 18 total errors remaining, all either the 7 dead-code `squads` errors (deliberately left — zero callers anywhere in `src/`, flagged for a future deletion pass, not a live-bug fix) or the 11 pre-existing `src/db/*` script errors CLAUDE.md already treats as known/acceptable. **Zero errors in live, reachable application code.**
- Observed result: full file list and fixes — `AuthButton.tsx` (session/custom-auth union type narrowing on avatar/image), `BottomNav.tsx` + `docs/page.tsx` (nav/section `icon` typed `React.ElementType` instead of `LucideIcon`, caused prop-type collapse to `never`), `MatchComponents.tsx` (`Match` type was imported from `@/lib/mock-data`, which never exported it — no local type existed there at all; corrected to the real shared `Match` type at `@/types`, matching every other component's import convention), `admin/players/page.tsx` (uncontrolled/controlled `<select>` value fix, `teamId ?? ''`), `admin/teams/[id]/page.tsx` (CSV-import `Resolution` union type wasn't narrowing inside a nested JSX callback — extracted a locally-narrowed `resolvedPlayerId` const before the JSX return), `api/ads/route.ts` (nullable `impressions` guarded before increment), `api/basketball/standings/route.ts` + `api/football/standings/route.ts` + `api/other/matches/route.ts` (same `or()` possibly-`undefined` pattern already fixed once for `matches/route.ts` in commit `1468b58` — applied the same non-null-assert pattern, condition is never actually undefined at these call sites since both `eq()` args are always concrete), `api/head-to-head/route.ts` (the zero-match fallback object and `calculateH2HStats`'s return were both missing `competitionId`/other fields present on the real `headToHead` row type — completed both shapes), `api/staff-comms/route.ts` (imported `uuid`, an **undeclared transitive dependency** not in `package.json` at all — swapped to `nanoid()`, matching this codebase's actual id-generation convention used everywhere else in `src/app/api/`, removing the phantom dependency entirely rather than adding `@types/uuid`), `admin/news/page.tsx` (`EnhancedNewsEditor`'s `setFormData` prop was typed `(data: any) => void`, which can't contextually type a `prev => (...)` functional updater — retyped to `React.Dispatch<React.SetStateAction<FormData>>` matching what's actually passed in from the parent's real `useState`; also removed a dead `onRemove` prop on `<ImageUpload>` that was never part of `ImageUploadProps` and never wired to anything — `MobileImageUpload`'s internal remove button already calls `onChange('')` directly).
- **One genuine functional bug found, not just a type hole:** `src/components/ui/mobile-image-upload.tsx` passed Cloudinary's `croppingAspectRatio` widget option a display string (`'1:1'`, `'16:9'`, `'9:16'`) instead of the numeric ratio the widget actually requires (`1`, `16/9`, `9/16`) — meaning image-crop aspect-ratio constraining has silently never worked for any upload using this component, for any aspect-ratio setting, since it was written. Fixed by adding a separate `getAspectRatioNumber()` alongside the existing display-string `getAspectRatio()` (kept for the "Aspect ratio: 1:1" UI label), and pointing `uploadOptions.croppingAspectRatio` at the numeric version.
