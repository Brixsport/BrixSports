...
- **Deliberately not touched:** `emailNotifications`/`favoriteTeamUpdates`/`weeklyDigest` toggles remain in the UI but still have zero backing system — wiring them would mean building three new features (an email pipeline, per-team notification filtering, a digest job), not fixing a bug. Left as an open decision for Richard: build the backing systems, or remove/relabel these specific toggles as "Coming soon" so they stop implying a working feature. Not decided or built this session.
