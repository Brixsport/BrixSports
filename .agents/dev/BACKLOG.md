...
- **BACKLOG-105** _(HIGH — Data Integrity)_: Penalty shootout score isolation. Full implementation. **Status:** SHIPPED — 2026-08-04 (session 48), core mechanics live-verified; the 3 gaps below (`BUG-197`, `BACKLOG-190`, `BACKLOG-191`) fixed session 51 continuation, 2026-08-20 — see each for evidence.
  ...
  **`BACKLOG-191`** _(Test coverage gap)_: **Status:** RESOLVED (partial) — 2026-08-20. Session 48's live verification jumped directly into a match seeded at `PENALTY_SHOOTOUT` (via direct DB insert) rather than testing the real full pipeline — normal time (0-0 or tied) → extra time → still tied → shootout. **This session:** drove a real throwaway tied match through the actual `PATCH /api/matches/[id]` transitions `FootballLogger.tsx` fires (`SECOND_HALF` → `EXTRA_TIME_1` → `EXTRA_TIME_2` → `PENALTY_SHOOTOUT`) against staging as a real authenticated logger, confirmed each PATCH `200`s and the final DB row lands correctly in `PENALTY_SHOOTOUT` with the match still `LIVE`. This exercises the server-side period-transition path end-to-end, which is what this entry was actually worried about (the older, previously-tested `TD-010` logic hadn't been re-confirmed reachable since this session's shootout changes landed). **Not covered:** the literal UI button-click sequence through `FootballLogger.tsx`'s period-transition confirms (tested via direct API PATCH, matching what those buttons call, not by clicking them) — same reasoning as `BACKLOG-190` above for not doing a full UI click-through this session.
