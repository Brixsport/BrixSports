...
- **BUG-108** _(CRITICAL — Real-Time / Flow C, session 42, found during a live match test on staging)_: **Live event broadcast to public viewers and the DB write for that event are two fully independent, uncoordinated actions — nothing ties them together.** Confirmed directly:
  ...
  **Suspected root cause, not yet confirmed**: the Railway dashboard (checked live for BUG-074, same session) showed **"0 Variables"** configured for the `ws-server` service specifically. If `WS_API_KEY` is genuinely unset there, `ws-server/index.js`'s check (`apiKey !== process.env.WS_API_KEY`) would reject every `/broadcast` call with a silent 401 — caught by `src/lib/socket.ts`'s own try/catch (`console.warn`, server-side only, not visible from the browser), matching every symptom observed exactly. **Session ended before this could be confirmed** — next session's first task is checking Railway's `ws-server` Variables tab directly and comparing `WS_API_KEY` against what Vercel has configured, then redoing this same live test.
  **Status:** SHIPPED, live-tested, **appears NOT working** — do not treat as resolved or even as a confirmed-working mitigation. This is the single most important open thread from this session.
...
- **BUG-116** _(CRITICAL — Real-Time / Flow B, session 43, split out from BUG-108's "Fix (not yet built)" section)_: **The real fix for BUG-108 — moving the WS broadcast onto the server, triggered after a confirmed DB write.** `server.js`'s `event:log` handler (referenced in BUG-108) is a pure relay with no DB access.
  ...
  **Live test result, end of session**: does NOT appear to be working — see BUG-108's entry for the full live-test detail. Suspected cause: Railway's `ws-server` service shows "0 Variables" configured, `WS_API_KEY` likely unset there, meaning `/broadcast` calls are probably 401'ing silently. Not yet confirmed — session ended before Richard could check the Railway dashboard's Variables tab directly.
  **Status:** SHIPPED, NOT confirmed working. BUG-074's real fix (in the correct file, `ws-server/index.js`) also remains open — the `server.js` fix from earlier this session protects nothing live.
