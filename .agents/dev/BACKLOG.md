...
- **BUG-074** _(HIGH — Staging / Security)_: Staging and prod share the same Railway WebSocket server instance. Originally filed as "wrong WS URL in staging env vars" — root cause is deeper: there is only one Railway service, so staging and prod are on the same Socket.IO process. Confirmed consequences (2026-07-01):
  ...
  **Status:** BUG-074 stays OPEN — this closes the specific live-broadcast leakage risk demonstrated by BUG-108's testing this session, not the bug's full original scope. Deployed via push to `dev` (Railway's `ws-server` service tracks `dev` — confirmed with Richard). Not yet live-verified cross-environment (would need two simultaneous Origin contexts, staging + prod, to confirm isolation) — next session's first check if this comes back up.
...
- **BUG-108** _(CRITICAL — Real-Time / Flow C, session 42, found during a live match test on staging)_: **Live event broadcast to public viewers and the DB write for that event are two fully independent, uncoordinated actions — nothing ties them together.** Confirmed directly:
  ...
  **Evidence:**
  ...
  - Observed result: live broadcast delivered end-to-end for the first time this project has confirmed; ~7–17s latency observed, exceeds the <5s target — not blocking this bug's resolution but flagged as a follow-up
  - Pending items: latency investigation (see caveat above); BUG-074's real fix (environment-scoping `ws-server/index.js`, the correct deployed file) remains open, separate item
