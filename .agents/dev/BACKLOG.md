...
**Status:** IN PROGRESS (session 46, 2026-07-23) — basketball's core write path (score persistence, quarter/period persistence, the missed-shot stat bug, a roster-resolution bug blocking lineup selection entirely) fixed and pushed on `feature/basketball-write-path` (PR #11, targets `dev`, not yet merged). Live-verified via direct API calls + DB queries against the real PR preview for: score persistence (home_score 0->6 correctly), currentPeriod transitions (Q1->Q2->Q3->Q4->OT->FINISHED), quarter labels rendering correctly on the public page (screenshot evidence). **Still unverified:** the actual "Finalize Match reachable" fix (item 3 below) and the event-save failure banner both require a real interactive logger walkthrough, not API calls — in progress directly with Richard as of this entry. `TrackLogger.tsx`'s complete absence of a persistence layer (separate problem below) is untouched, still fully OPEN. See `RUNLOG.md` 2026-07-21/2026-07-23 for full verification evidence and `known-issues.md`/TD-012/BACKLOG-124 for adjacent findings discovered along the way (an admin-POST logger_id FK bug, a `second:0` collapsing to null, a local-dev-only ratings-fetch hang, and the Admin "Official Match Lineups" page's own separate football-centric-starter-count bug — none of those fixed, all filed separately).
