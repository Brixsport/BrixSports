...
- Not yet reconfirmed with a fresh live-fire after the middleware fix deploys (Railway's own poller will exercise it for real on its next 5-minute cycle once this ships — that's the actual confirmation, not a manual test).
- Pending items: watch Railway's deploy logs after this ships to confirm the "[Reminders] staging check failed" line stops appearing, and confirm the still-pending `reminder-live-208-*` test row (created earlier this session) actually gets marked sent on the next real cycle.
