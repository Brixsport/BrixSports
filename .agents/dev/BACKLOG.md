...
- **BACKLOG-107** _(HIGH — PWA / iOS)_: Offline queue drain fallback for iOS. Background Sync API (`sync.register`) is not supported on iOS — the SW drain never fires on iPhone. Fix: add `window.addEventListener('online', drainQueue)` and `document.addEventListener('visibilitychange', drainQueue)` handlers in `FootballLogger.tsx` that drain `BrixsportAdminDB.pendingMatchEvents` directly from the page context. This is the cross-platform solution; Background Sync on Android becomes a secondary fallback only. **Must land before first real match with an iOS logger.** See `.agents/dev/PWA_LIMITATIONS.md` for full context. Filed: 2026-06-25. **Status:** OPEN
...
- **BUG-073** _(LOW — Data)_: Substitution `detail` field direction is inverted. The string is built as `"{outPlayer} IN for {inPlayer}"` but the correct English is `"{inPlayer} IN for {outPlayer}"`. The display UI reads `playerId`/`relatedPlayerId` directly and shows the correct direction — this is cosmetic only. But the raw event `detail` string in the DB is misleading and will confuse any future parsing. Confirmed in KIN vs JOG test match events. Fix: swap the string construction in the substitution detail builder in `FootballLogger.tsx`. Filed: 2026-06-25. **Status:** OPEN
