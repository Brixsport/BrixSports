...
### BACKLOG-208 — Consolidate the Two Reminder Routes + Fix `reminders/check` Bugs (Scheduler Was Already Live via Railway, Roadmap Doc's Research Was Incomplete)

**Status:** RESOLVED — 2026-08-06 (session 50, branch `feature/notification-system`)
**Priority:** Medium — downgraded from the roadmap doc's "High (unblocker)" once the actual premise was found to be wrong (see below); the real bugs fixed here are still worth having fixed, just not scheduler-blocking

**Corrected premise:** the roadmap doc (`NOTIFICATION_SYSTEM_ROADMAP_PROPOSAL.md` thread 2, session 49) claimed "neither reminder route has ever run in production" because `vercel.json` has no `crons` block. **That was wrong.** `ws-server/index.js:584-625` (Railway, `BACKLOG-150`'s merge, commit `b767416` — an earlier session, not this one) already runs `checkAllMatchReminders()` on a real 5-minute `setInterval`, calling `POST /api/reminders/check` for **both** staging and prod with their correct per-env `CRON_SECRET_STAGING`/`CRON_SECRET_PROD`. The background research agent that wrote the roadmap doc searched `src/` for the scheduler and never looked in `ws-server/`, a sibling directory for the separately-deployed Railway service — a real research gap, not a code gap. The reminder pipeline has been live since `BACKLOG-150` shipped, well before this session's notification work started.
...
- Not live-fire tested: the resolved-team-names fix in the reminder body wasn't confirmed via an actual on-device notification this pass (would require creating a real due reminder and either waiting for Railway's next 5-minute poll or triggering `POST` manually with the real `CRON_SECRET`, which — unlike the short-lived scoped JWTs used elsewhere this session — is a static shared credential; not exposed via a browser-visible tool call without a fresh explicit go-ahead for that specific tradeoff).
- Pending items: none blocking. The team-name fix is a straightforward, low-risk change (same shape as `BUG-200`'s already-proven pattern) that will get exercised for real the next time any real user's reminder comes due, given Railway's poller is already live and unaffected by any of this session's changes.
