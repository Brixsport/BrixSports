...
**Next session — exact first task:** genuinely open between two strong candidates — root-cause BUG-119's remaining ~9.9s latency (time/code-sensitive, Richard's stated priority), or start basketball write-path verification (bigger unknown than continuing the football backfill). Richard to confirm which.
