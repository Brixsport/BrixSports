...
**Deferred, explicitly:**
- `BUG-190` (settings auto-save redesign) — carried over from 47F untouched this session, still needs live re-verification of the new auto-save UI.
- `BUG-194` part 2 (football's full offline-queue feature parity) and `BACKLOG-060` (per-route caching strategy) — the two candidate starting points for the next "offline/cache pass, both sports" session; Richard's choice deferred to session start rather than picked here.
- `BACKLOG-131` (install-prompt cross-role dismissal bug), `BACKLOG-107` (iOS background-sync live device test), `BACKLOG-166` sub-finding 3 (competition-level foul threshold, needs a schema migration) — all confirmed still open via a re-read of the existing session 47D PWA/offline audit, none touched this session.

**PR / branch state:** all of this session's work is on `feature/basketball-live-clock`, pushed to `origin` through commit `e282901`. Basketball tier 0 is fully closed out. `/wrap`'s own documentation edits (this entry, `known-issues.md`, `BACKLOG.md` status updates) are local-only at time of writing — Richard asked not to push yet.

**Next session — exact first task:** confirm with Richard which of the two offline/cache candidates to start with — `BUG-194` part 2 (football offline-queue feature parity, a live-test-required behavior change) or `BACKLOG-060` (caching strategy overhaul) — then proceed with the same live-test-first workflow this session established (DB/route ground truth before marking anything RESOLVED, never UI-inferred alone).
