...
**Picking back up on:** finish Phase 2 — extend `NOTIFIABLE_EVENT_TYPES`/period mapping/`match-notification-service.ts`'s type union for basketball's quarter-end, game-end, and a minimal, deliberately-non-spammy scoring/foul event set (informed by, but not blocked on, the parallel roadmap agent's basketball-UX section), then live-verify the same way Phase 1 was verified. Once the two background agents land: review `BUG-201`'s root-cause findings and fix it; review the roadmap proposal with Richard and batch-file whatever's agreed into `BACKLOG.md`.
