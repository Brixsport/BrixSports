...
- **BUG-074 — staging and prod share the same Railway WS server.** ~~Room namespacing is `match:{matchId}` only — no environment prefix.~~ **Workaround shipped and live-verified, session 44** — every room (`staging:match:{id}` vs `prod:match:{id}`), the global goal notification, and the in-memory clock cache are now env-prefixed, ported to the actually-deployed `ws-server/index.js`. **BUG-074 itself stays OPEN, not fully resolved**: shared `JWT_SECRET` across environments is untouched, and the originally-recommended real fix (a second, independent Railway service for staging) still hasn't been built. Cross-environment isolation itself is logically-reviewed but not live-verified against real dual-environment traffic. Full detail in `BACKLOG.md` BUG-074.
...
| BUG-074 staging/prod WS room collision | 0 — workaround shipped, not fully resolved | Session 44 workaround live-verified (room-prefix, notification, and cache scoping ported to the real deployed `ws-server/index.js`); shared `JWT_SECRET` and the recommended second-Railway-service fix remain open. See `BACKLOG.md` BUG-074. |
