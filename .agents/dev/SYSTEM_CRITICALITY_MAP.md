...
- **Logger WS emits are unauthenticated at the socket level.** ~~Railway WS server is a pure passthrough with no token validation on connection.~~ **BUG-120, session 44 — fixed, not yet deployed.** `ws-server/index.js` now verifies a logger JWT via Socket.IO's `auth` option at handshake (env-aware: staging and prod sign with different secrets, same shared Railway instance, so the right one is selected via the existing Origin-based env detection from BUG-074). Logger-mutation events (`event:log`, `match:time:update`, and ~12 others) are gated behind it; viewer sockets remain correctly unauthenticated. Verified locally with real Socket.IO test clients (`dev/test-ws-logger-auth.mjs`, `dev/test-ws-env-secret-selection.mjs`). Needs `JWT_SECRET_STAGING`/`JWT_SECRET_PROD` added to Railway before deploy — see `BACKLOG.md` BUG-120 for full detail. This is also a prerequisite for single-writer enforcement (see `LIVE_CLOCK_V2_ARCHITECTURE.md §5`) — you cannot validate an assignment without knowing who the socket belongs to, and now the socket layer actually can.
...
| Logger WS socket auth at handshake | 0 | BUG-120, session 44 — fixed locally, not yet deployed (needs Railway env vars). Prerequisite for single-writer enforcement. |
