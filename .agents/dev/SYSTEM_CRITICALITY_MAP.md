...
- **BUG-074 — staging and prod share the same Railway WS server.** ~~Room namespacing is `match:{matchId}` only — no environment prefix.~~ **Workaround shipped and live-verified, session 44** — every room (`staging:match:{id}` vs `prod:match:{id}`), the global goal notification, and the in-memory clock cache are now env-prefixed, ported to the actually-deployed `ws-server/index.js`. **BUG-074 itself stays OPEN, not fully resolved**: the originally-recommended real fix (a second, independent Railway service for staging) still hasn't been built, and cross-environment isolation itself is logically-reviewed but not live-verified against real dual-environment traffic. (`JWT_SECRET` sharing is NOT part of what remains — secrets were already rotated and separated per environment 2026-07-01, see the "JWT secret rotation" row below; an earlier version of this note wrongly listed it as still open.) Full detail in `BACKLOG.md` BUG-074.
...
- ~~**Logger WS emits are unauthenticated at the socket level.**~~ **BUG-120, session 44 — RESOLVED, live-verified.** `ws-server/index.js` now verifies a logger JWT via Socket.IO's `auth` option at handshake (env-aware: staging and prod sign with different secrets, same shared Railway instance, so the right one is selected via the existing Origin-based env detection from BUG-074). Logger-mutation events (`event:log`, `match:time:update`, and ~12 others) are gated behind it; viewer sockets remain correctly unauthenticated. Live-verified against the real deployed Railway instance and a real connected viewer tab (`dev/test-ws-logger-auth-live.mjs`) — unauthenticated `event:log` rejected, a real logger JWT succeeded and reached the viewer live. Full detail in `BACKLOG.md` BUG-120. Known remaining limitation: authentication only, not per-match authorization — a valid logger could still trigger a live broadcast for a match they're not assigned to (REST persistence already blocks the actual write). This was also a prerequisite for single-writer enforcement (see `LIVE_CLOCK_V2_ARCHITECTURE.md §5`) — you cannot validate an assignment without knowing who the socket belongs to, and now the socket layer actually can.
