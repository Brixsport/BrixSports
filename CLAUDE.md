...
- [ ] Public page updates within 5 seconds of event save — BUG-109 (clock DB fallback) RESOLVED session 43. BUG-108/116 (broadcast never firing — root cause was `NEXT_PUBLIC_WS_URL` missing its `https://` scheme on Vercel) RESOLVED session 44, live-tested end-to-end with a genuine WS push confirmed (`[WS] New event received...`, no reload). **Still not fully checked off — latency is real and worse than first thought**: an initial browser-side estimate of ~7–17s turned out to be an unreliable eyeballed guess. A later test's actual Railway server logs showed a 42-second gap between DB write and broadcast. Root cause not yet investigated (candidates: Railway cold path, Socket.IO room-emit delay, something upstream of the broadcast call itself). Next investigation must use server-log timestamps, not browser-side tool-call timing.
