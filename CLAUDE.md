...
- [ ] Public page updates within 5 seconds of event save — SHIPPED (BUG-080 polling fallback `74ca73b`). **REGRESSION CONFIRMED, session 42** — live staging test found two real gaps: (1) BUG-108, offline-queued events that sync later never trigger a live broadcast to already-open viewers, only a manual refresh reveals them; (2) BUG-109, the live clock number has no DB fallback at all and freezes permanently (survives even a refresh) once the logger's socket disconnects. Neither is the Railway-down scenario this line was originally waiting to verify — both are worse, confirmed on a live logger/viewer connection with no server outage involved.
