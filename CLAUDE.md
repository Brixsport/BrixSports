...
- Lineup Builder — **re-audited session 51/53**: `BUG-219` (publish crash), `BUG-220` (no publish path + unenforced lock), `BUG-221` (unauthenticated draft-lineup leak) all RESOLVED and live-verified. `BACKLOG-220`'s architecture cleanup (dead duplicate rendering code, non-atomic write race, no formation-change confirm) is now also RESOLVED (session 2026-09-05, write-race live-tested with a real concurrent request, dead code deleted, tsc clean) — kept 🔴 conservatively pending a full re-audit rather than downgraded outright, since no fresh session has yet re-verified the feature end to end against this specific list of past bugs
...
- [x] Two simultaneous loggers do not conflict or overwrite — RESOLVED session 2026-09-10. Live-tested with 2 real logger accounts firing the identical goal simultaneously (`Promise.all`, real concurrency) against the deployed preview: found a real race (both passed `BUG-196`'s dedup guard, score double-counted), fixed (dedup check moved inside the same transaction as the insert, `BACKLOG-151`), re-tested on a fresh match post-fix — confirmed closed (second logger's duplicate correctly rejected, correct score). Clock collision risk (Directive 6) not covered by this specific test — separate concern, still worth a dedicated check before a real multi-logger match day.
...
- [x] Public page updates within 5 seconds of event save — RECONFIRMED 2026-09-18 via `tests/smoke/critical-flows.ts`'s automated Flow C check: 758ms measured (target <5000ms), first-ever automated (not manual/stakeholder-report) measurement of this line. Prior manual measurements (~9.9s, "roughly ten seconds") predate this and are superseded — see `BACKLOG-400` for the smoke test itself.
...
- [ ] Track & Field logging: **NOT SAFE, zero persistence** — `TrackLogger.tsx` saves nothing today (`BACKLOG-400` item 4, deliberately deferred alongside the Basketball/Track shared-core extraction). Do not run a real Track & Field event through this logger; a live event today has no DB write path at all.
